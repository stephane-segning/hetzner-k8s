...
| D-4   | ~~Node passwords (`<nodename>.node-password.k3s` Secrets) can conflict on reboot/re-replace/restore~~ **CLOSED** | Caused all 3 workers `NotReady` on 2026-06-02                                          | **Resolved** by ADR-0012: cloud-init pre-seeds a deterministic node password. Break-glass remediation documented in `recovery.md` |
