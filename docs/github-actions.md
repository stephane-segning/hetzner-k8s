...
- Keep etcd snapshot backups in a separate bucket or prefix from Terraform state.
...
- `TF_ETCD_S3_ENABLED`: defaults to `false`
...
- `ETCD_S3_BUCKET`: optional bucket for etcd snapshots
- `ETCD_S3_ENDPOINT`: optional S3 endpoint for etcd snapshots, for example `nbg1.your-objectstorage.com`
